...
class ArticleService
{
        ...
        if ($request->ajax()) {
            ...
            if (empty($request->search['value'])) {
                if (auth()->user()->hasRole('owner')) {
                    $data = Article::latest()
                        ...
                        ->limit($limit)
                        ->withTrashed()
                        ->get(['id', 'uuid', 'title', 'category_id', 'views', 'published', 'deleted_at', 'is_confirm']);
                } else {
                    $data = Article::latest()
                        ->with('category:id,name', 'tags:id,name')
                        ->offset($start)
            ...
            return DataTables::of($data)
                ...
                ->editColumn('is_confirm', function ($data) {
                    return $data->is_confirm ? '<span class="badge bg-success">Confirm</span>' : '<span class="badge bg-danger">Unconfirm</span>';
                })
                ...
                ->rawColumns(['title', 'category_id', 'tag_id', 'published', 'action', 'views', 'is_confirm'])
                ->with([
    ...
    public function delete(string $uuid)
    {
        ...
        $getArticle->tags()->updateExistingPivot($getArticle->tags, ['deleted_at' => now()], true);
    ...
    {
        $getArticle = $this->getFirstBy('uuid', $uuid);
        $getArticle->update(['is_confirm' => 1]);
