...
class ArticleService
{
    ...
    public function popularArticles()
    {
        ...
        ->select('id', 'user_id', 'category_id', 'title', 'slug', 'published', 'is_confirm', 'views', 'image', 'published_at')
        ->orderBy('views', 'desc')
        ->where('published', true)
        ->where('is_confirm', true)
        ->limit(4)
    ...
    public function articleByCategory($category = 'default-category')
    {
        $articles = Article::with('category:id,name,slug', 'user:id,name')
            ->whereHas('category', function ($query) use ($category) {
                $query->where('slug', $category);
    ...
    public function articleSingle($category = 'default-category')
    {
        $articles = Article::with('category:id,name,slug', 'user:id,name')
            ->whereHas('category', function ($query) use ($category) {
                $query->where('slug', $category);
    ...
    {
        $articles = Article::with('category:id,name', 'user:id,name')
        ->select('id', 'user_id', 'category_id', 'title', 'slug', 'published', 'is_confirm', 'image', 'published_at')
        ->where('published', true)
        ->where('is_confirm', true)
        ->limit(2)
        ->get();
